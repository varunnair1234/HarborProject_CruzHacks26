...
class CashFlowEngine:
    ...
    def compute_metrics(
        ...
        """
        Compute all cash flow metrics from daily revenue data
        
        Args:
            daily_revenues: List of {date, revenue} dicts
        ...
        revenues = np.array([r["revenue"] for r in daily_revenues])
        data_days = len(revenues)
        
        # Basic statistics
        avg_daily = float(np.mean(revenues))
        ...
        # Fixed cost burden
        total_fixed_monthly = (
            fixed_costs.get("rent", 0) +
            fixed_costs.get("payroll", 0) +
            fixed_costs.get("other", 0)
        )
        fixed_cost_burden = (
            total_fixed_monthly / avg_monthly if avg_monthly > 0 else float("inf")
        )
        
        # Runway (if cash_on_hand is provided)
        cash_on_hand = fixed_costs.get("cash_on_hand")
        runway_days = None
        if cash_on_hand is not None and avg_daily > 0:
            # Simple runway: cash / (daily avg revenue - daily avg costs)
            daily_fixed_costs = total_fixed_monthly / 30
            net_daily_cash_flow = avg_daily - daily_fixed_costs
            if net_daily_cash_flow > 0:
                runway_days = None  # Positive cash flow, infinite runway
            else:
                # Burning cash
                daily_burn = abs(net_daily_cash_flow)
                runway_days = float(cash_on_hand / daily_burn) if daily_burn > 0 else None
    ...
    def _compute_trend(revenues: np.ndarray, days: int) -> float:
        ...
        avg_newer = np.mean(newer_half)
        
        if avg_older == 0:
            return 0.0
        
        return float(((avg_newer - avg_older) / avg_older) * 100)
    ...
    def _compute_confidence(data_days: int, volatility: float) -> float:
        ...
        if data_days >= 90:
            ...
            data_confidence = 0.5 + (data_days - 30) / 60 * 0.2
        else:
            data_confidence = 0.3 + (data_days / 30) * 0.2
        ...
        return min(data_confidence + volatility_confidence, 1.0)
